...
struct thread_pack {
	...
	unsigned short *s_reg;
	unsigned char *s_coil;
};
...
// Using this as Modbus TCP respond fc = 0xf/0x10
struct tcp_frm{
	unsigned short transID;
	unsigned short potoID;
	unsigned short msglen;
...
struct mbus_tcp_func{
	...
	int (*build_1516_resp)(struct tcp_frm*, struct thread_pack*, unsigned char);
};
